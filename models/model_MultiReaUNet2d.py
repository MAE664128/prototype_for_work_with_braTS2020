...
class Model2DMultiResUnet:
    """ модель 2D-MultiRes UNet """
    ...
    def __init__(self,
        ...
        self.model = self.get_2d_multires_u_net_model(depth=self.depth,
    ...
    @staticmethod
    def get_2d_multires_u_net_model(input_shape: tuple,
                                    n_classes: int,
        ...
        for layer_depth in range(depth - 2, -1, -1):
            ...
            up_convolution = Model2DMultiResUnet.get_up_convolution(input_layer=current_layer,
                                                                    pool_size=pool_size,
    ...
    def get_multi_res_path(input_layer, n_filters, length_of_path,
        ...
        shortcut = tf.keras.layers.Conv2D(n_filters,
                                          kernel_size=(1, 1),
                                          padding=padding,
                                          strides=strides)(input_layer)
        shortcut = tf.keras.layers.BatchNormalization()(shortcut)

        layer = tf.keras.layers.Conv2D(n_filters, kernel,
        ...
        for m in range(length_of_path - 1):
            ...
            shortcut = tf.keras.layers.Conv2D(n_filters, kernel_size=(1, 1),
    ...
    @staticmethod
    def get_multi_res_block(input_layer, n_filters, kernel=(3, 3),
                            padding='same', strides=(1, 1)):
        # fsf = filter scaling factor
        fsf = 1.67
        weight = fsf * n_filters

        shortcut = tf.keras.layers.Conv2D(int(weight * 0.167) + int(weight * 0.333) + int(weight * 0.5),
                                          kernel_size=(1, 1),
                                          padding=padding,
                                          strides=strides)(input_layer)
        shortcut = tf.keras.layers.BatchNormalization()(shortcut)

        layer1 = tf.keras.layers.Conv2D(int(weight * 0.167), kernel,
                                        padding=padding, strides=strides, activation='relu')(input_layer)
        layer1 = tf.keras.layers.BatchNormalization()(layer1)

        layer2 = tf.keras.layers.Conv2D(int(weight * 0.333), kernel,
                                        padding=padding, strides=strides, activation='relu')(layer1)
        layer2 = tf.keras.layers.BatchNormalization()(layer2)

        layer3 = tf.keras.layers.Conv2D(int(weight * 0.5), kernel,
    ...
    def get_up_convolution(input_layer, n_filters, pool_size, kernel_size=(2, 2), strides=(2, 2),
        ...
        if type_up_convolution == "up_sampling_2d+conv2d":
            ...
            l = tf.keras.layers.Conv2D(n_filters, (2, 2), padding="same", strides=(1, 1))(l)
    ...
    def load_model(path_to_model_file, n_classes=None, compile=True):
        if n_classes is None:
            n_classes = Model2DMultiResUnet.N_CLASSES
        print("Loading pre-trained model")
        custom_objects = get_jaccard_metric_as_custom_object(n_classes)
        custom_objects["InstanceNormalization"] = InstanceNormalization
        return tf.keras.models.load_model(path_to_model_file, custom_objects=custom_objects, compile=compile)
